...
public class SocketIOGameRoom extends GameRoom {
  ...
  public SocketIOGameRoom(User user, URI gameServerURL, String code) {
    ...
    socket.on("game turn action", args -> {
      dispatchGameTurnAction();
    });
    socket.on("game turn listen", args -> {
      dispatchGameTurnListen();
    });
    socket.on("game turn listen category submission", args -> {
      dispatchGameTurnListenCategorySubmission((Integer) args[0]);
    });
    socket.on("game turn listen question option submission", args -> {
      dispatchGameTurnListenQuestionOptionSubmission((Integer) args[0]);
    });
    socket.on("game finished", args -> {
      releaseGameLock();
    });
    socket.on("room terminated", args -> {
      releaseGameLock();
    });
    socket.connect();
  ...
  public void waitForGameToFinish() {
    synchronized (lock) {
      try {
        ...
        throw new RuntimeException("Application lock was terminated: " + e.getMessage());
      }
    }
  }
