...
public class ChangeCategorySettingScreenProvider implements ScreenProvider {
    ...
    public void execute() throws InterruptedException {
        ...
        while (categoriesPerUserInGame == 0) {
            NumberInputScreen numberInputScreen = screenFactory.createNumberInputScreen("How many categories do you want to play per user in one game?", "Enter a number");
            ...
            if (categoriesPerUserInGame > 0) {
                this.repository.getSettingsRepository().setCategoriesPerGameAndUser(categoriesPerUserInGame);
